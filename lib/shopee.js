...
// ต้องขอครบทุกสถานะที่หลังร้านโชว์ — รุ่นแรกขอแค่ NORMAL/UNLIST แท็บ "การละเมิด" กับ
// "อยู่ระหว่างตรวจสอบ" ในหน้า /product เลยว่างตลอด
// SHOPEE_DELETE = Shopee ลบให้เพราะผิดกฎ (เช่นสแปมราคา) หลังร้านยังโชว์ในแท็บการละเมิด ต้องเอามาด้วย
// ไม่เอา SELLER_DELETE — ร้านลบเอง หลังร้านไม่โชว์แล้ว
export async function listItemIds({ accessToken, shopId, partner = null, statuses = ['NORMAL', 'UNLIST', 'BANNED', 'REVIEWING', 'SHOPEE_DELETE'] }) {
  const out = [];
...
export function normalizeListing(base, models, shop) {
  ...
  return {
    listing: {
      ...
      // "ถูกลดการมองเห็น" — ยังขายอยู่ (NORMAL) แต่หลังร้านนับในแท็บการละเมิด ตอบมาเป็นข้อความ "TRUE"/"FALSE"
      deboost: base.deboost === true || String(base.deboost).toUpperCase() === 'TRUE',
...
// เหตุผลที่ติดการละเมิด — ถามเฉพาะตะกร้าที่ถูกแบน/ลดการมองเห็น/ถูกลบ (ร้านหนึ่งมีไม่กี่ตัว)
// คืน Map(item_id → { name, status, details: [{type, reason, suggestion}] })
// name/status ใช้กับตะกร้าที่ Shopee ลบแล้ว — get_item_base_info ไม่ตอบตัวพวกนี้เลย
export async function getViolations({ accessToken, shopId, itemIds, partner = null }) {
  const out = new Map();
  for (let i = 0; i < itemIds.length; i += 50) {
    const data = await call('/api/v2/product/get_item_violation_info', {
      accessToken, shopId, partner, params: { item_id_list: itemIds.slice(i, i + 50).join(',') },
    });
    for (const it of data.item_list || []) {
      out.set(String(it.item_id), {
        name: it.item_name || null,
        status: it.item_status || null,
        // ลดการมองเห็นอยู่ใน deboost_details · แบน/ถูกลบอยู่ใน item_status_details
        details: [...(it.item_status_details || []), ...(it.deboost_details || [])].map((d) => ({
          type: d.violation_type || null,
          reason: d.violation_reason || null,
