...
const FIELDS = [
  'item_list', 'package_list', 'pay_time', 'ship_by_date', 'total_amount', 'order_status',
  'recipient_address', 'buyer_username', 'cancel_by', 'cancel_reason', 'update_time', 'create_time',
].join(',');
...
// Shopee ไม่ส่ง "เวลากดจัดส่ง" กับ "เวลาขนส่งรับ" มาตรงๆ เหมือน TikTok
// แต่บอกสถานะพัสดุมาให้ อนุมานจากตรงนั้นแทน
...
export function normalizeOrder(o, shop) {
  ...
  return {
    order: {
      ...
      // ไม่มีเวลาจริง ใช้เวลาที่ออเดอร์ขยับล่าสุดเป็นตัวประมาณ
      rts_at: readyToPick || pickedUp ? updated : null,
      collected_at: pickedUp ? updated : null,
