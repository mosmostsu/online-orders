...
export async function fetchMonth(month) {
  ...
  // ไฟล์ส่งออกมาจาก pandas บางเดือนมี NaN (บรรทัดที่ไม่มีจำนวน/ราคา) — เป็น JS ที่ถูก แต่ JSON.parse ไม่รับ
  // ม.ค. 2568 มี 3 จุด ทำให้ทั้งเดือนหายไปเงียบๆ แปลงเป็น null ก่อน (บรรทัดพวกนี้ถูกข้ามตอนคิดทุนอยู่แล้ว)
  const body = txt.slice(txt.indexOf('['), txt.lastIndexOf(']') + 1)
    .replace(/:\s*-?(NaN|Infinity)(?=\s*[,}\]])/g, ': null');
  try {
    return JSON.parse(body);
  } catch (e) {
    throw new Error(`อ่านไฟล์ ${month} ไม่ได้: ${e.message}`);
  }
}
