...
export async function pushText(text) {
  ...
  const [tg, line] = await Promise.all([pushTelegram(text), pushLine(text)]);

  // ถือว่าสำเร็จถ้าถึงปลายทางอย่างน้อยหนึ่งทาง — ตอนนี้ LINE โควตาเต็มแต่ Telegram ส่งได้
  // ก็ไม่ควรให้ทั้งใบถูกนับว่าแจ้งไม่สำเร็จแล้วส่งซ้ำไม่จบ
  const ok = Boolean(tg.ok || line.ok);
  const skipped = !telegramReady() && line.skipped ? true : undefined;
...
// ใส่เงื่อนไขช่องทางให้คิวรีของ Supabase — คืนคิวรีเดิมถ้าตั้งไว้ว่าเตือนทุกช่องทาง
export function onlyNotifyPlatforms(q) {
  const list = notifyPlatforms();
  return list ? q.in('platform', list) : q;
}
